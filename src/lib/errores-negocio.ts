...
export function manejarErrorNegocio(error: unknown): NextResponse {
  if (error instanceof EntidadNoEncontradaError) return NextResponse.json({ error: error.message }, { status: 404 });
  if (error instanceof EstadoInvalidoError) return NextResponse.json({ error: error.message }, { status: 409 });
